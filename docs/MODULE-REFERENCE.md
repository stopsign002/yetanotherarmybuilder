...
- **Storage:** `localStorage.yaab_show_allies` (`'1'` / `'0'`, **defaults off** since 2026-09-24; it defaulted on from launch until then). The chip's own state is **not** persisted — it always starts off (`null`) on load; see `docs/UI.md` "Roster filter chips — and the Ally chip".
- **DOM:** `#yaab-btn-allies`, `.unit-card-ally`, `.detail-ally-tag`, `.ally-chip` (in `#roster-filter-chips`, before `.filter-chips-clear`).
- **Notes:** Presentation only — the units themselves are attached by `attachAlliedUnits()` in `js/data/dc-adapter.js`, which stamps `_allyOf` / `_allyLabel` / `_allySourceFaction` / `_allyRuleIds` / `_allyDetachments` on a per-host **clone**. Like the Legends toggle it defaults OFF (owner's call, 2026-09-24 — allied clones crowd every host roster; it launched defaulting ON because allies had been invisible). The detail tag is emitted inline by `js/ui/detail.js`, not via MutationObserver. The "Ally" chip mirrors `favorites.js`'s injection pattern (MutationObserver on `#panel-center`, inject once, disconnect) but cycles through 3 states like roster.js's own keyword chips (off → include `.active` → exclude `.excluded` → off) rather than a plain boolean; it also listens on the bar's `.filter-chips-clear` button to reset its own state, since that button only resets DOM classes and has no idea the chip's JS state variable exists. The one `rosterFilters` predicate combines the chip and the toolbar toggle so they can't contradict each other — chip `include` always wins over the toggle.
...
### `js/ui/play-mode.js`
- **Purpose:** Play mode — fast-switching game-day reference (owns `#play-mode`). Persistent unit switcher + Sheets / Stratagems / Rules / Enhancements tabs, everything pre-rendered so switching is class-toggling only. Desktop Sheets tab has a layout toggle: "One" (switcher rail, one sheet visible) or "All" (every datasheet in one horizontal, sideways-scrolling row) — phones always get "One". Light tracking: CP counter, per-entry destroyed toggle.
- **Exports:** `App.openPlayMode` (entry point), `App.playMode.{activate, refresh}`.
- **Depends on:** `UI.renderUnitDetail` / `UI.renderRuleDetail` with `opts.host` + `opts.gameView` (every surface — datasheets, stratagems, rules, enhancements — is the build-mode Details pane, builder chrome stripped); `App.CardRenderers` (the data facade `cards-mode.js` exposes over its gatherers — deduped multi-detachment unions); `App.hooks.{modeChange, armyChange, selectionChange, bootstrap, armyToolbarActions}`, `App.state`, `UI.escapeHtml`; `window.matchMedia('(min-width: 821px)')` to keep the effective layout in sync with the app's single breakpoint.
- **Storage:** `localStorage.yaab_play_view` (active tab + layout preference + last sheet per army — device-local), `yaab_play_game` (per-army CP + per-entry destroyed state, capped at the 10 most-recently-touched armies — device-local, deliberately NOT cloud-synced).
- **DOM:** `#play-mode` host. All content renders `.unit-detail-content` markup into `.play-detail-host` divs (per sheet, and per stratagem/rule/enhancement grid item), compacted by `#play-mode`-scoped rules in `css/play-mode.css`.
- **Notes:** Lazy-built on first activate; re-renders on army/selection change (dirty-flagged while hidden). Stacked entries (`count > 1`) always split into one virtual sheet per copy — tracking keys are `entryId` for copy 1 and `entryId::i` for the rest, so single-copy entries keep their stored state. Game state is keyed by those ids so it survives re-renders and army edits. Entry points: the top-bar shelf "Play" button (`btn-play-mode`, whitelisted in `TOPBAR_SHELF_IDS`) and the `go-play` Settings-drawer row (both widths).
